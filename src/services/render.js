...
exports.contactus = (req, res) => {
	res.render("contactus", { logged: login_status });
};

exports.train = (req, res) => {
	res.render("train", { logged: login_status });
};
...
exports.trainbookingSuccessful = async (req, res) => {
	...
	try {
		...
		if (isMatch && password === cpassword) {
			...
			res.render("trainTicket", { tik: ticket, logged: login_status });
		} else {
			// res.send("password are not matching");
			return sendResponse("password are not matching", res, 201);
		}
	} catch (error) {
...
exports.flightbookingSuccessful = async (req, res) => {
	...
	try {
		...
		if (isMatch && password === cpassword) {
			...
			res.render("flightTicket", { tik: ticket, logged: login_status });
		} else {
			// res.send("password are not matching");
			return sendResponse("password are not matching", res, 201);
		}
...
exports.searchTrain = async (req, res) => {
	try {
		...
		// }
		res.render("searchTrain", { trains: tbs, logged: login_status });
	} catch (err) {
		// res.status(201).send("Invalid details...");
		return sendResponse("Invalid details...", res, 201);
	}
};
...
exports.searchFlight = async (req, res) => {
	try {
		...
		// }
		res.render("searchFlight", { flights: fbs, logged: login_status });
	} catch (err) {
		// res.status(201).send("Invalid details...");
		return sendResponse("Invalid details...", res, 201);
	}
...
exports.flight = (req, res) => {
	res.render("flight", { logged: login_status });
};

exports.bus = (req, res) => {
	return sendResponse("Sry, This servies is under construction", res, 201);
};

exports.trainbookingForm = (req, res) => {
	res.render("trainbookingForm", { logged: login_status });
	// console.log(req.query.id);
};

exports.flightbookingForm = (req, res) => {
	res.render("flightbookingForm", { logged: login_status });
	// console.log(req.query.id);
};

exports.loginRegister = (req, res) => {
	res.render("loginRegister", { msg: "", logged: login_status });
...
exports.signin = async (req, res) => {
	try {
		...
		if (isMatch) {
			...
			return sendResponse(
				"Invalid Login details...password not match",
				res
			);
		}
	} catch (err) {
	...
	try {
		...
		let msg = sendMail(
			...
			.then((msg) => {
				// console.log("hi");
				// console.log(msg);
				res.status(201).render("loginRegister", {
					msg: msg,
					logged: login_status,
				});
			})
